        ...
        foreach ($localeDirs as $localeDir) {
            ...
            if ($language) {
                ...
                foreach ($langFiles as $langFile) {
                    $group = str_replace(array('/', $localeDir . DIRECTORY_SEPARATOR, '.php'),
                        array(DIRECTORY_SEPARATOR, '', ''), $langFile);
                    $lines = $this->fileLoader->loadRawLocale($locale, $group);
                    $this->languageEntryProvider->loadArray($lines, $language, $group, null,
